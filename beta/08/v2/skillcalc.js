...
function drawSkilltree(id) {
    ...
    for(i=0;i<skill[id].length;i++) {
        ...
        if(lang == 0 && skillname.toString().length >= 20) {
            skillname = "<span style='letter-spacing:-1px'>"+skillname+"</span>";
        } else if(lang == 1 && skillname.toString().length >= 13 && skillname.toString().length < 16) {
            skillname = "<span style='letter-spacing:-2px'>"+skillname+"</span>";
        } else if(lang == 1 && skillname.toString().length >= 16) {
            skillname = "<span style='letter-spacing:-3px'>"+skillname+"</span>";
        }
...
//    8 ABL
    ...
    if(mainclass==7 || subclass==7) {
        ...
        tmpstats[2] += skillstats[1][cursp[7][10]]+skillstats[1][cursp[7][11]];
    ...
    if(document.getElementById("ch_mag").checked == true) {
        ...
        if(mainclass==7 || subclass==7) {
            //bouncer mag
            if(cursp[7][6]>0) {
                tmpstats[2] += Math.floor((25+(cursp[7][6]*5))*0.01*mag[3]);
                tmpstats[4] += Math.floor((25+(cursp[7][6]*5))*0.01*mag[3]);
            }
        }
    }
...
function decode(code) {
    ...
    if(codever<8){
        tmpurl = document.location.href;
        tmpurl = tmpurl.substring(0,tmpurl.indexOf("skillcalc.php"))+"old/0"+codever+"/skillcalc.php"+code;
        tmpmsg = "Unable to load. Code is incompatible with current version.";
        tmpmsg += "<br><a href='"+tmpurl+"'>You can use an older version of the simulator to recover the build by using this link.</a>";
        layerOn(getStringTranslation(0,57),tmpmsg,1);
        return;
